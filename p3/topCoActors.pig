...
-- Keep only actor records
acted1 = FILTER principal_movies BY (category == 'actor' OR category == 'actress');
acted2 = FILTER principal_movies BY (category == 'actor' OR category == 'actress');
...
ordered_data = ORDER count_data BY co_count DESC;
top_25 = LIMIT ordered_data 25;

-- Join with the names to get actor's names
join_name1 = JOIN top_25 BY Actor1, names BY nid;
name1 = FOREACH join_name1 GENERATE names::primaryName AS Actor1 , top_25::Actor2 AS Actor2 , top_25::co_count AS co_count;
